...
if (mysqli_num_rows($res11) > 0) {
        ...
        header("refresh:2;url=local_authority_signup_validation.php");        
        exit();
...
if ($result) {
   ...
   echo '<script>alert("The login Credentials are send to Email Sucessfully");window.location="add_local_auth.php";</script>';
   
} else {
    echo mysqli_error($conn);
    echo '<script>alert("Oops.. Something went wrong! Please try again...");window.location="add_local_auth.php"";</script>';
}
?>
