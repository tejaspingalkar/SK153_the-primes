...
$query = "UPDATE `embankment` SET `embankment_name` = '$nameofembank', `location` = '$location', 
`latitude` = '$latitude', `longitude` = '$longitude', 
`date_of_establishment` = '$dateofest',`river` = '$river', `area` = '$river', `diameter` = '$diameter', 
`thickness` = '$thickness', `length` = '$length', `density` = '$density', `youngs_modulus` = '$youngmod', 
`pile_spacing` = '$pilespacing', `crssec_area_of_pile_cap` = '$csaofpc', `poissons_ratio` = '$poissonratio', 
`state` = ' $state',`district` = '$district',`taluka` = '$taluka',`village` = '$village',`image` = '$path' WHERE `embankment_id` = '$eid'";
...
if ($result) {
   // echo mysqli_error($conn);
    //echo $path;
   echo '<script>alert("Values Edited Sucessfully");window.location="index.php";</script>';
} else {
    //echo mysqli_error($conn);
    echo '<script>alert("Oops.. Something went wrong! Please try again...");window.location="index.php";</script>';
}
